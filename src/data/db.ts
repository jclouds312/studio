import type { Job, CompanyProfile, User, SubscriptionPlan } from '@/lib/types';

// This file serves as a temporary, in-memory "database" by exporting
// the static data arrays. In a real application, these would be replaced
// by database queries using an ORM like Prisma.

export { allJobs } from './jobs';
export { allUsers } from './users';
export { allCompanies } from './companies';
export { allPlans } from './plans';
